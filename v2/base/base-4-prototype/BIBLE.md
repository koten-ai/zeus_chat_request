...
| **Folder** | `v2/base/base-4-prototype/` |
...
| 8 | [`PROTOTYPE.md`](PROTOTYPE.md) | Short diet changelog only |
...
A **chat_request** is the LLM **catalog** for one agent mode: house rules + 13 Zeus tools (verbs) + **terminate contract** (how the turn must end). base-4 is a **dieted prototype** of that catalog: same verbs and required evidence fields as before, but **one clear Terminate table + example** instead of two long overlapping essays. Zeus Client loads a catalog, injects live scope data, runs multi-round tool chat, and parses a final **Layer A** `return` object for UI, control flags, and admin metrics.
