...
| **Theme** | Client emit **`user`** + settings **`ai_process_result`** (cheap default) |
...
1. **CORE** — short Client-loop note:
   ...
   - Report sinks stamp root **`user`**: `zeus_client` | `zeus` | `helios` | `admin` (product Client always `zeus_client`).
...
| ID | Owner |
| --- | --- |
| **ZC-WISH-035** | zeus_client stamps `user` on sinks |
